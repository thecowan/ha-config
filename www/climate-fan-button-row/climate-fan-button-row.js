...
window.customCards.push({
  type: "climate-fan-button-row",
  name: "climate fan button row",
  description: "A plugin to display your climate's fan controls in a button row.",
  preview: false,
});

class CustomClimateFanModeRow extends Polymer.Element {
	...
	hassChanged(hass) {
		...
		if (custModes) {
			if (stateObj && stateObj.attributes) {
				if (stateObj.state == 'on' && stateObj.attributes.fan_mode == m1 ) {
					mode1 = 'on';
				} else if (stateObj.state == 'on' && stateObj.attributes.fan_mode == m2 ) {
					mode2 = 'on';
				} else if (stateObj.state == 'on' && stateObj.attributes.fan_mode == m3 ) {
					mode3 = 'on';
				} else {
					offstate = 'on';
				}	
			...
			if (stateObj && stateObj.attributes) {
				if (stateObj.attributes.fan_mode == "Low" ) {
					mode1 = 'on';
				} else if (stateObj.attributes.fan_mode == "Mid" ) {
					mode2 = 'on';
				} else if (stateObj.attributes.fan_mode == "High" ) {
					mode3 = 'on';
				} else {
					offstate = 'on';
				}	
	...
	setMode(e) {
		...
		if(mode == 'off' ){
			...
			if (this._config.sendStateWithMode) {
				...
				param.fan_mode = this._modeOne;
				this.hass.callService('climate', 'set_fan_mode', param);
			} else if (mode == 'mode2') {
				param.fan_mode = this._modeTwo;
				this.hass.callService('climate', 'set_fan_mode', param);
			} else if (mode == 'mode3') {
				param.fan_mode = this._modeThree;
				this.hass.callService('climate', 'set_fan_mode', param);
			}
		}
	}
...
customElements.define('climate-fan-button-row', CustomClimateFanModeRow);
